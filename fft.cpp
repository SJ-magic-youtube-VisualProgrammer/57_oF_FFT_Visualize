...
{
	int sgn = 1;
	if(Gui_Global->dir_ArtSin_FW)	sgn = 1;
	else							sgn = -1;
	
	for(int i = 0; i < phase_rad_madeFromGain.size(); i++){
	/*
		if(Gui_Global->dir_ArtSin_FW)	phase_rad_madeFromGain[i] = ofMap( Gain_smoothed_2[i], 0.0, 1.0, 0.0, 2 * _PI );
		else							phase_rad_madeFromGain[i] = ofMap( Gain_smoothed_2[i], 0.0, 1.0, 0.0, -2 * _PI );
	*/
		
		if(i == 1)	phase_rad_madeFromGain[i] = ofMap( Gain_smoothed_2[i], 0.0, 1.0, 0.0, sgn * Gui_Global->ArtSin_PhaseMap_k * _PI );
		else		phase_rad_madeFromGain[i] = ofMap( Gain_smoothed_2[i], 0.0, 1.0, 0.0, sgn * 2 * _PI );
	}
